...
/* This example accompanies the books
 ...
 sund : 25-02-2018 Implement Lab2 from Realtime Bluetooth Networks 
 */
...
int main(void){
  ...
  TExaS_Init(GRADER, 6493);          // initialize the Lab 2 grader
  //TExaS_Init(LOGICANALYZER, 6493); // initialize the Lab 2 logic analyzer
...
// Task5 will stall
  ...
  // Task2, Task3, Task4, Task5 are main threads
  OS_AddThreads5(&Task2, &Task3, &Task4, &Task5);
  // when grading change 1000 to 4-digit number from edX
  TExaS_Init(GRADER, 1000);          // initialize the Lab 2 grader
//  TExaS_Init(LOGICANALYZER, 1000); // initialize the Lab 2 logic analyzer
  OS_Launch(BSP_Clock_GetFreq()/THREADFREQ); // doesn't return, interrupts enabled in here
