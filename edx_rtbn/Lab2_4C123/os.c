...
void(*ptask1)(void);
void(*ptask2)(void);
uint32_t ptask1_period = 0;
uint32_t ptask2_period = 0;
static uint32_t count_systicks = 0;
...
void SetInitialStack(int i){
  //***YOU IMPLEMENT THIS FUNCTION*****
	tcbs[i].sp = &Stacks[i][STACKSIZE-16]; // Top of the stack
	Stacks[i][STACKSIZE - 1] = 0x01000000; // Thumb bit is the 24th bit of the EPSR ( Execution program status register) PSW
	// PC not set here STACKSIZE - 2
	
	// Just set random values to the registers and build the stack
...
int OS_AddThreads5(void(*thread0)(void),
                  void(*thread1)(void),
                  void(*thread2)(void),
                  void(*thread3)(void)){
...
// initialize four stacks, including initial PC
  ...
  const int num_tasks = 4;									 
	...
	/* for the last task set it to first */
	tcbs[i].next = &tcbs[0];
	SetInitialStack(i);
	
	/* setting the program counter for each task */
	Stacks[0][STACKSIZE-2] = (int32_t)task0;
	Stacks[1][STACKSIZE-2] = (int32_t)task1;
	Stacks[2][STACKSIZE-2] = (int32_t)task2;
  Stacks[3][STACKSIZE-2] = (int32_t)task3;
	
	/* setting the current run pointer*/
	RunPt = &tcbs[0];
...
int OS_AddPeriodicEventThreads(void(*thread1)(void), uint32_t period1,
  ...
  //***YOU IMPLEMENT THIS FUNCTION*****
		...
		ptask1 = thread1;
		ptask2 = thread2;
		ptask1_period = period1;
		ptask2_period = period2;
...
void Scheduler(void){ // every time slice
  ...
  //***YOU IMPLEMENT THIS FUNCTION*****
	// R0-R3 passed as argumented. Int results stored in R0 accoring to AAPCS, in this case we do not use it

	count_systicks = count_systicks + 1;
	
	// Bad pointer handling needs to be done
	/*if(ptask1 ==  || ptask2 == NULL)
	{
		// Bad pointer. Should raise exception here
	}*/

	if(ptask1_period > ptask2_period)
	{
		count_systicks = count_systicks % ptask1_period;
	}
	else
	{
		count_systicks = count_systicks % ptask2_period;
	}	
	if((count_systicks % ptask1_period) == 0)
	{
		ptask1();
	}
		if((count_systicks % ptask2_period) == 0)
	{
		ptask2();
	}
...
// Inputs:  none
// Outputs: none
int32_t semPt; // Semaphore used for the Mail box
uint32_t mailbox_data; // Data Field for the Mail box
void OS_MailBox_Init(){
  // include data field and semaphore
  OS_InitSemaphore(&semPt,0);  // Init semaphore to zero
	mailbox_data = 0;                  // Init data to zero
